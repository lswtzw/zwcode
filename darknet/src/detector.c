...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dont_show, int calc_map, int mjpeg_port, int show_imgs)
{
    ...
    int savew_step = 3;//#flag#_zwnewgflag
    while (get_current_batch(net) < net.max_batches )
    {
        ...
        */
        ...
        //这里是每迭代100次以后，保存一次最后权重文件。
        //目前一次的时间约为100秒，所以3*100=300=5分钟，可以存一次为佳
        //if (i >= (iter_save_last + 100) || i % 100 == 0)  //#flag#zwnewg_modi
        if (i >= (iter_save_last + savew_step) || i % savew_step == 0)
        {
            iter_save_last = i;
#ifdef GPU
            if (ngpus != 1) sync_nets(nets, ngpus, 0);
        ...
        //#flag#zwnewg_add
        FILE *zfp=NULL;  fopen_s(&zfp, "c:\\savew.txt", "r");
        if (zfp == NULL)break;//end while 删除标志文件时，退出while,以后可以继续开始的。
        char fbuf[5] = { 0 };
        fread_s(fbuf, 4, 4, 1, zfp);
        savew_step = atoi(fbuf);        
        fclose(zfp);
        printf("\n**************************\n");
        printf("savew_step = %d", savew_step);
        printf("\n**************************\n");
        //#flag#zwnewg_add
    }//end while
    ...
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box)
{
    ...
    ////////////zwnewg#flag#move out from while(1){ #start#;......
    if (filename)
    {
        ...
        fflush(stdout);
        input = fgets(input, 256, stdin);//手工输入文件名
        if (input) {
            strtok(input, "\n");
        }
    }
    ...
    ////zwnewg#flag#add
    //范例:image/zw1001.jpg -> image/zw1002.jpg -> image/zw1003.jpg .....9999.jpg
    int ilen = strlen(input);
    char numbuf[32] = { 0 };
    int numi = -1;
    if (ilen >= 8)
    {
        if ((input[ilen - 8] >= 49 && input[ilen - 8] <= 57)
            && (input[ilen - 7] >= 48 && input[ilen - 7] <= 57)
            && (input[ilen - 6] >= 48 && input[ilen - 6] <= 57)
            && (input[ilen - 5] >= 48 && input[ilen - 5] <= 57))
        {
            memcpy(numbuf, &input[ilen - 8], 4);
            numi = atoi(numbuf);
        }
    }
    ////zwnewg#flag#add
    ...
    {       
        ...
        if (!dont_show) {
            //show_image(im, "predictions");
             show_image(im, input);//#zwnewg#flag#modi
        }
        ...
        //zwnewg#flag#add
        //范例:image/zw1001.jpg -> image/zw1002.jpg -> image/zw1003.jpg .....9999.jpg
        if (numi >= 1000 && numi<9999)
        {
            numi++;
            _itoa(numi, numbuf, 10);
            memcpy(&input[ilen - 8], numbuf, 4);//文件名中的数字增量+1
        }
        else break;//end while
        //zwnewg#flag#add

        //if (filename)break;//zwnewg#flag#del
    }//end while
