...
layer parse_deconvolutional(list *options, size_params params)
{
    ...
    //注意:实际上bn,pad都要加进去的,但现在这些都没做,没起作用
    layer l = make_deconvolutional_layer(batch, h, w, c, n, size, stride, padding, activation);
...
crop_layer parse_crop(list *options, size_params params)
{
    ...
    float angle = option_find_float(options, "angle",0);//这个单位应为%5,0.05之类的,但看代码又不是,像是1~360度
    ...
    l.shift = option_find_float(options, "shift", 0);//这个是平移,单位未确定
    l.noadjust = noadjust;
    return l;
}
...
dropout_layer parse_dropout(list *options, size_params params)
{
    ...
    //注意:上面表示,默认值是丢弃率为20%
    //注意:darknet中的这个和tensorflow中的定义是相反的.tensorflow中定义的是保留率
    int dropblock = option_find_int_quiet(options, "dropblock", 0);//这个设置为1,将是另一种drop模式
...
route_layer parse_route(list *options, size_params params)
{
    ...
    int i;
    for(i = 0; i < len; ++i){
        if (l[i] == ',') ++n; //n是要合并的总层数,按","号来计数,最小的n也是初值1层
    }
    ...
    for(i = 0; i < n; ++i){
        int index = atoi(l);
        l = strchr(l, ',')+1;
        if(index < 0) index = params.index + index;//当前层-1,就是上一层
        layers[i] = index;
        sizes[i] = params.net.layers[index].outputs;
    }
    int batch = params.batch;

    //下面看来ROUTE还可以分组,并设置ID
    int groups = option_find_int_quiet(options, "groups", 1);
...
//这个是最核心的开头的[net]解析!注意:它不是第0层!,它是一个全局定义
...
//zw_flag# 下面这三个参数是否应设置一个初值,非0的!???
    net->h = option_find_int_quiet(options, "height",0);
    net->w = option_find_int_quiet(options, "width",0);
    net->c = option_find_int_quiet(options, "channels",0);//1,or ,3

    net->inputs = option_find_int_quiet(options, "inputs", net->h * net->w * net->c);
    net->max_crop = option_find_int_quiet(options, "max_crop",net->w*2);
    net->min_crop = option_find_int_quiet(options, "min_crop",net->w);
    //注意:下面的参数是INT型,其实就是BOOL型开关
    net->flip = option_find_int_quiet(options, "flip", 1);//翻转-镜像
    net->blur = option_find_int_quiet(options, "blur", 0);//模糊
    net->mixup = option_find_int_quiet(options, "mixup", 0);//混合
    int cutmix = option_find_int_quiet(options, "cutmix", 0);
    int mosaic = option_find_int_quiet(options, "mosaic", 0);//镶嵌
    ...
    //估计下面是数据增广用到的
    net->angle = option_find_float_quiet(options, "angle", 0);//从全部代码分析可知,它是0~360度之间的角度,估计超过了也不报错
    net->aspect = option_find_float_quiet(options, "aspect", 1);//侧面/切面(可否理解为PS中的斜切?)
    net->saturation = option_find_float_quiet(options, "saturation", 1);//色饱和度
    net->exposure = option_find_float_quiet(options, "exposure", 1);//曝光
    net->hue = option_find_float_quiet(options, "hue", 0);//色调
...
void save_weights_upto(network net, char *filename, int cutoff)
{
    ...
    for(i = 0; i < net.n && i < cutoff; ++i){
        ...
        if(  ( l.type == CONVOLUTIONAL || l.type == DECONVOLUTIONAL ) && l.share_layer == NULL){
   ...
   fflush(fp);//************only add it zw for mseg.c
    fclose(fp);
}
