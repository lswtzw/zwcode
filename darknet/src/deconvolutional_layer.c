...
size_t get_deconv_workspace_size(deconvolutional_layer l) {
    return (size_t)l.h*l.w*l.size*l.size*l.n * sizeof(float);
}


//void denormalize_deconvolutional_layer(deconvolutional_layer l)
//{
//    int i, j;
//    for (i = 0; i < l.n; ++i) {
...
deconvolutional_layer make_deconvolutional_layer(int batch, int h, int w, int c, int n, int size, int stride, int padding, ACTIVATION activation)
{
    int i;
    ...
    //l.pad = padding;
    //int out_h=l.out_h = (l.h - 1) * l.stride + l.size - 2 * l.pad;
    //int out_w=l.out_w = (l.w - 1) * l.stride + l.size - 2 * l.pad;
