...
list *get_paths(char *filename)
{
    ...
    while((path=fgetl(file))){
        //////////////zw add #flag#
        if (path == NULL ||path[0] == 0 || path[0] == 0x0a || path[0] == 0x0d)continue;
        //////////////zw add #flag#
...
void *load_thread(void *ptr)
{
    ...
    } else if (a.type == UNET_DATA) {//注,因为img和label必须同步变化,所以它的数据增广要另外做,不能使用现有的代码.
        *a.d = load_data_unet(a.n, a.paths, a.m, a.w, a.h, a.c);
    }
...
extern LPZWDATAFILE pzwdf; 
//zwadd for unet.c #flag#
//注意:下面这个函数没有做数据增广,所以后面传入的一大堆参数是没有意义的,相反,可以参考classifier.c的代码.它做了,在LOAD数据时.
//这里只能用python等第三方外部工具做数据增广,因为要求的是:img,label两张对应图像,必须,同步变换!!!***
//所以此函数的重点注意是:保持IMG和LABEL的数据"一一对应"
data load_data_unet(int n, char **paths, int m, int w, int h, int channels)
{
    char *label_images = pzwdf->train_label_image_path;//"C:/unet_data/train/label/";
    char *train_images = pzwdf->train_image_path;// "C:/unet_data/train/image/";
    

    char train_img[256];
    char label_img[256];
    char **random_paths = get_random_paths(paths, n, m);//n样本总数,m每批的批量
    int i;
    data d = { 0 };
    d.shallow = 0;
    ...
    for (i = 0; i < n; ++i) {
        //下面生成确保同步对应关系的两个全路径文件名
        sprintf(train_img, "%s%s%s", train_images, random_paths[i], pzwdf->image_file_type_hz);
        sprintf(label_img, "%s%s%s", label_images, random_paths[i], pzwdf->image_file_type_hz);
        //printf("Train img file:%s\n", train_img);
        //printf("Train lab file:%s\n", label_img);

        image orig, label;
        if (channels <= 1)//灰度与rgb的区别
        {
            orig = load_image_gray(train_img, 0, 0);
            label = load_image_gray(label_img, 0, 0);
        }
        else
        {
            orig = load_image_color(train_img, 0, 0);
            label = load_image_color(label_img, 0, 0);
        }
